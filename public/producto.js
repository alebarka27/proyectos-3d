        ...
        main.innerHTML = `
            ...
            <div class="producto-similares">
                ...
                <div class="eshop-grid">${data.similares.map(s => {
                    const sFoto = mlHighResImage(fotosArray(s.fotos)[0]);
                    const sImg = sFoto ? `<img src="${escapeHTML(safeHref(sFoto))}" alt="${escapeHTML(s.nombre)}" loading="lazy">` : `<div class="product-img-placeholder">${icon('printer', 'icon-lg')}</div>`;
